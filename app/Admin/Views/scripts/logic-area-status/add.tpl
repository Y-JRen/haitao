...
<div class="title">仓储管理 -&gt; {{$area_name}} -&gt; 商品状态申请</div>
<div class="content">
...
<tbody>
    ...
    <tr>
      <td colspan="2">
      <input type="button" onclick="openDiv('{{url param.controller=product param.action=sel param.type=sel_stock param.logic_area=$logic_area}}','ajax','查询商品',750,400);" value="查询添加商品">
      </td>
    </tr>
</tbody>
</table>
...
function addRow()
{
	...
	for (i = 1; i < el.length; i++) {
		if (el[i].checked) {
		    ...
		    tr.cells[7].innerHTML = '<input type="text" size="6" name="number[]" id="number' + batch_id + '" value="0" class="required" msg="不能为空" onblur="checkNum(this, ' + pinfo.able_number + ')"/>';
		    obj.appendChild(tr);
		}
	}
}
