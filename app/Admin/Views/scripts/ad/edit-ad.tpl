...
<tr>
     <th width="80">广告名称 :</th>
     <td><input type="text" size="40" value="{{$adInfo.name}}" class="input-text" id="name" name="name"><div id="nameTip" class="onShow">请填写广告名称</div></td>
</tr>

    <tr>
    <th>广告链接 :</th>
     <td><input type="text" size="40" class="input-text"  value="{{$adInfo.url}}" name="url"></td>
   </tr>
    
    <tr><th>广告位 :</th><td><select id="board_id" name="board_id">
    {{foreach from=$adBoard item=item}}
    <option  {{if $adInfo.board_id eq $item.id}}selected{{/if}}  tpl="{{$item.tpl}}"   allowtype="{{$adtpl[$item.tpl].allow_type}}" value="{{$item.id}}">{{$item.name}}（{{$item.width}}*{{$item.height}}）</option>
    {{/foreach}}
    </select></td></tr>
    
   <tr><th>广告类型 :</th><td>
   <select id="type" name="type">
    {{html_options options=$adType selected=$adInfo.type}}
   </td>
   </tr>
  
  <tr class="bill_media" id="ad_image" style="display: table-row;">
  <th>广告图片 :</th>
  <td><input type="text" size="30" class="input-text fl mr10 tips" id="J_img" value="{{$adInfo.content}}"  name="image">
  ...
  <th>广告代码 :</th><td><textarea id="code" name="code" cols="50" rows="3">{{$adInfo.content}}</textarea></td></tr>
  <tr style="display:none;" class="bill_media" id="ad_flash">
  <th>广告动画 :</th>
  <td><input type="text" size="30" class="input-text fl mr10" id="J_flash"  value="{{$adInfo.content}}" name="flash">
  ...
  <th>文字内容 :</th><td><textarea id="text" name="text" cols="50" rows="3">{{$adInfo.content}}</textarea></td></tr>
  ...
  <tr class="bill_media" id="moreimg" style="display:none;">
    ...
    <p>
    <div><span class="fl">1. 链接：</span> <input type="text" size="30" class="input-text fl mr10" value="{{$config[0].link}}" name="extConfig[0][link]"></div>
    <div><span class="fl"> 图片：</span><input type="text" size="30" class="input-text fl mr10 tips" id="J_more_1"  value="{{$config[0].pic}}"  name="extConfig[0][pic]"></div> 
   ...
   <input type="file" name="file" style=" font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;"></div>
    ...
    <div><span class="fl">2. 链接：</span> <input type="text" size="30" class="input-text fl mr10" value="{{$config[1].link}}"   name="extConfig[1][link]"></div>
    <div><span class="fl"> 图片：</span><input type="text" size="30" class="input-text fl mr10 tips" id="J_more_2"  value="{{$config[1].pic}}"  name="extConfig[1][pic]"></div> 
   <div class="upload_btn" id="J_upload_more_2" style="position: relative; overflow: hidden; direction: ltr;"><span>上传</span>
   <input type="file" name="file" style=" font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;"></div>
   </p>
      
    <p style="clear:both;padding-top:10px;">
    <div><span class="fl">3. 链接：</span> <input type="text" size="30" class="input-text fl mr10" value="{{$config[2].link}}"   name="extConfig[2][link]"></div>
    <div><span class="fl"> 图片：</span><input type="text" size="30" class="input-text fl mr10 tips" id="J_more_3" value="{{$config[2].pic}}"  name="extConfig[2][pic]"></div> 
  ...
  <tr><th>扩展图片 :</th><td><input type="text" size="30" class="input-text fl mr10 tips" id="J_extimg" name="extimg" value="{{$adInfo.extimg}}">
  <div class="upload_btn" id="J_upload_extimg" style="position: relative; overflow: hidden; direction: ltr;"><span>上传</span>
  <input type="file" name="file" style="font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;"></div></td></tr>
  <tr><th>扩展字段值 :</th><td><input type="text" class="input-text fl mr10" name="extval" value="{{$adInfo.extval}}"></td></tr>
  <tr><th>广告描述 :</th><td><input type="text" size="30" class="input-text fl mr10" name="desc" value="{{$adInfo.desc}}"></td></tr>
  <tr><th>广告时间 :</th><td><input type="text" size="20" class="Wdate"  value="{{$adInfo.start_time|date_format:'%Y-%m-%d %H:%M:%S'}} "  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"  id="start_time" name="start_time"> - 
  <input type="text" size="20" class="Wdate"   onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="{{$adInfo.end_time|date_format:'%Y-%m-%d %H:%M:%S'}}" id="end_time" name="end_time"></td></tr>
  <tr>
  
  <th>是否启用 :</th>
   <td><label><input type="radio" {{if $adInfo.status eq 1}}checked=""{{/if}} value="1" name="status">是</label>&nbsp;&nbsp;
<label><input type="radio" {{if $adInfo.status eq 0}}checked=""{{/if}}  value="0" name="status">否</label></td></tr>
   </tbody>
 </table>
</div>

<div class="submit">
<input type="hidden" value="{{$adInfo.id}}" name="id" />
