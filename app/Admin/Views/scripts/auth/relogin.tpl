<div class="login_main">
    <ul class="login_box">
    <form name="loginForm" id="loginForm" method="post" action="{{url}}" onsubmit="return validate()" target="ifrmSubmit">
    ...
    <li>验证码：<input name="verifyCode" type="text" size="12" maxlength="5" style="width:70px" onkeyup="this.value = this.value.toUpperCase();"/>
        <img class="auth-image" src="{{url param.action=auth-image param.space=adminLogin}}" alt="verifyCode" border="0" onclick= this.src="{{url param.action=auth-image  param.space=adminLogin param.code=}}"+Math.random() style="cursor: pointer;" title="点击更换验证码" />
	</li>
    <li>
