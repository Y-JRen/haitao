...
&nbsp;代销商：
<select name="user_name" id="user_name" style="width:80px">
  <option value="">请选择...</option>
  {{foreach from=$areas item=item key=key}}
    {{if $key > 20}}
    <option value="{{$distributionArea[$key]}}" {{if $param.user_name eq $distributionArea[$key]}}selected{{/if}}>{{$item}}</option>
    {{/if}}
  {{/foreach}}
</select>
&nbsp;结款状态：
...
<select name="distribution_type" id="distribution_type">
  <option value="">请选择...</option>
  <option value="1" {{if $param.distribution_type eq '1'}}selected{{/if}}>是</option>
  <option value="0" {{if $param.distribution_type eq '0'}}selected{{/if}}>否</option>
</select>
<br><br>
...
<div class="content">
    ...
    <tr>
        ...
        <td>
          {{foreach from=$areas item=item key=key}}
            {{if $key == $distributionAreaUsername[$data.user_name]}}
              {{$item}}
            {{/if}}
          {{/foreach}}
        </td>
        ...
        <td>{{if $data.distribution_type}}是{{else}}否{{/if}}</td>
        <td>
          {{if $data.clear_pay}}
            已结款
          {{else}}
