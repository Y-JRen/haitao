...
<div class="bread container">
	<ul>
		...
		<li><a href="/gallery-{{$nav.0.cat_id}}-0-0-0-1.html"><strong>{{$nav.0.cat_name}}</strong></a>></li>
		<li><a href="/gallery-{{$nav.1.cat_id}}-0-0-0-1.html"><strong>{{$nav.1.cat_name}}</strong></a>></li>
		<li><a href="javascript:void(0);">{{$data.goods_name}}</a></li>
	</ul>
</div><!--breadcrumbs-->
<div class="productDetail container">
	<div class="product-top">
      <div class="syFocusThumb">
      	<div class="jqzoom">
      		<a href="{{$imgBaseUr}}/{{$data.goods_img}}"  class="jqzoom" rel='gal1'  title="triumph">
      			<img src="{{$imgBaseUr}}/{{$data.goods_img|replace:'.':'_380_380.'}}" height="400" />      
      		</a>
      	</div>
      		...
      		<a href="javascript:;" onfocus="this.blur();"   class="iconbg btn_left  btn_left_disable ">
      			< </a>
      				...
      				<div class="spec_item" id="list_smallpic">
      					<ul id="thumblist">
      						<li>	
	                			<a onfocus="this.blur();" class="active"   class="active"  href="javascript:void(0)"  rel="{gallery: 'gal1', smallimage: '{{$imgBaseUr}}/{{$data.goods_img|replace:'.':'_380_380.'}}',largeimage: '{{$imgBaseUr}}/{{$data.goods_img}}'}" >
	                        		<img src="{{$imgBaseUr}}/{{$data.goods_img|replace:'.':'_60_60.'}}" height="56"  alt="{{$data.goods_name}}" >
	                       		</a>
	                		</li>
      						{{foreach from=$imgurl item=v}}
      						<li>
      							<a onfocus="this.blur();" href='javascript:void(0);' rel="{gallery: 'gal1', smallimage: '{{$imgBaseUr}}/{{$v.img_url|replace:'.':'_380_380.'}}',largeimage: '{{$imgBaseUr}}/{{$v.img_url}}'}" >
      								<img src="{{$imgBaseUr}}/{{$v.img_url|replace:'.':'_60_60.'}}"  width="57" height="57"  alt="{{$v.img_desc}}" >
      							</a>
      						</li>
      						{{/foreach}}
      					</ul>
		...
		<div class="product-info">
			...
			<h2>{{$data.goods_alt}}</h2>
			<div class="divider"></div>
			商品编号：{{$data.goods_sn}}<br>
			<span>品&nbsp;&nbsp;&nbsp;牌：<a href='/region-search-{{$data.brand_id}}-0-0-1.html'>{{$data.brand_name}}</a></span><span>产&nbsp;&nbsp;&nbsp;地：{{$data.region_name}}</span><br>
			<p class="product_info_price">市场价格：<i>￥{{$data.market_price}}</i></p>
            <p class="product_info_price">本店价格：￥{{$data.shop_price}}</p>
            <p class="product_info_tax">行邮税：￥{{$data.tax}}  {{if $data.tax le 50}}<span style='height:22px;line-height:22px;display:inline-block; color:#d7a21e;border:1px solid #f7e7d7;padding:0 5px;font-size:12px;background:#f6fbe7;'>单笔订单行邮税总额低于50元（含50元）予以免征</span>{{/if}}</p>
            <p class="product_info_price">商品总价：<b>￥{{$data.price}}</b></p>
            <div class="service"><i>服务承诺：</i><p></p></div>
            <div class="divider"></div>
            <div class="guige">
            	<strong>规格分类：</strong>
            	<input type='hidden'  value="{{$product.0.product_sn}}" id='product' name='product'/>
				{{foreach from=$product item=v}}
					<a class="guige" rel="{gallery: 'gal1', smallimage: '{{$imgBaseUr}}/{{$v.product_img|replace:'.':'_380_380.'}}',largeimage: '{{$imgBaseUr}}/{{$v.product_img}}'}">
						<img src="{{$imgBaseUr}}/{{$v.product_img|replace:'.':'_60_60.'}}" style='width:38px;height:38px;' val={{$v.product_sn}} class="product_type"  title="{{$v.goods_style}}"  />
					</a>
				{{/foreach}}
			</div>
            ...
            <div class="shuliang"><strong>购买数量：</strong>
             ...
             {{if $data.onsale eq 0 && $data.able_number neq 0}}
             ...
             {{elseif $data.onsale eq 0 && $data.able_number eq 0}}
            ...
            <div class="w_share">
	            ...
	            <div class="collection">
	                <a class="png_bg" href = "javascript:void(0)" onclick="favGoods(this,'{{$data.goods_id}}')">收藏 {{$data.count}}</a>
		    ...
		    <div class="Contentbox">
		        <div id="con_two_1">
		          <div class="attributes">
		           <div class="attribute">
		          	<p>商品名称：{{$data.goods_name|cut_str:25}}</p>
		          	<p>商品品牌：{{$data.brand_name}}</p>
		          	<p>商品编号：{{$data.goods_sn}}</p>
		          	<p>商品类型：{{$data.cat_name}}</p>
		          	<p>商品国别：{{$data.region_name}}</p>
		          	<p>上架时间：{{$data.goods_add_time|date_format:'%Y-%m-%d'}}</p>
		          	</div>
		          </div>
		         {{$data.description}}
		        </div>
		        <div id="con_two_2" style="display:none">
		            {{$data.introduction|stripslashes}}
		        </div>
		...
		<div class="productM-right">
		    ...
		    <div class="hot-pro rel-brand border1">
		     {{foreach from=$brand item=v}}
		     	<p><a href="/region-search-{{$v.brand_id}}-0-0-1.html">{{$v.brand_name}}</a></p>
		     {{/foreach}}
