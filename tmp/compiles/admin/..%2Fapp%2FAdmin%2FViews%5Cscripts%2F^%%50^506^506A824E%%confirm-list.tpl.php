<?php /* Smarty version 2.6.19, created on 2015-07-31 16:50:11
         compiled from transport/confirm-list.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'html_options', 'transport/confirm-list.tpl', 15, false),array('modifier', 'date_format', 'transport/confirm-list.tpl', 101, false),)), $this); ?>
<style type='text/css'>
...
<input type="button" name="dosearch2" value="所有被我锁定的运输单" onclick="ajax_search(this.form,'<?php echo $this -> callViewHelper('url', array(array('do'=>'search','is_lock'=>yes,)));?>','ajax_search')"/>
<input type="button" name="dosearch3" value="所有没有锁定的运输单" onclick="ajax_search(this.form,'<?php echo $this -> callViewHelper('url', array(array('do'=>'search','is_lock'=>no,)));?>','ajax_search')"/>
</div>
</form>
<?php endif; ?>
...
&nbsp;&nbsp;&nbsp;&nbsp;
<!-- 初始运单号：<input type="text" name="logistic_no" size="10" maxLength="50" value="<?php echo $this->_tpl_vars['param']['logistic_no']; ?>
"/> -->
<input type="button" value="填充运单号" onclick="<?php if ($this->_tpl_vars['param']['bill_type'] == '' || $this->_tpl_vars['param']['logistic_code'] == '' || $this->_tpl_vars['param']['is_cod'] == ''): ?>alert('请先选择单据类型/物流公司/付款方式');return;<?php endif; ?>ajax_submit(this.form, '<?php echo $this -> callViewHelper('url', array(array('action'=>"fill-no",)));?>','Gurl(\'<?php echo $this -> callViewHelper('url', array());?>\')')">
...
<!-- 初始发票号：<input type="text" name="invoice_no" size="8" maxLength="20" value="<?php echo $this->_tpl_vars['param']['invoice_no']; ?>
"/>
<input type="button" value="填充发票号" onclick="ajax_submit(this.form, '<?php echo $this -> callViewHelper('url', array(array('action'=>"fill-invoice-no",)));?>','Gurl(\'<?php echo $this -> callViewHelper('url', array());?>\')')">
<input type="button" value="打印发票" onclick="this.form.method='post';this.form.target='_blank';this.form.action='<?php echo $this -> callViewHelper('url', array(array('action'=>"prints-invoice",)));?>';this.form.submit()"> -->
...
<input type="button" style='background:#b30e00 none repeat scroll 0 0;color:#ffffff;"' value="导出订单" onclick="this.form.method='post';this.form.target='_blank';this.form.action='<?php echo $this -> callViewHelper('url', array(array('controller'=>'transport','action'=>'export','act'=>"confirm-list",)));?>';this.form.submit()"">
...
" <?php if ($this->_tpl_vars['data']['lock_name'] == $this->_tpl_vars['auth']['admin_name']): ?>class='my_tr'<?php endif; ?>>
        <td><input type="checkbox" name="ids[<?php echo $this->_tpl_vars['data']['tid']; ?>
]" value="<?php echo $this->_tpl_vars['data']['tid']; ?>
" onclick='chooseId(<?php echo $this->_tpl_vars['data']['tid']; ?>
,this)' style='width:19px;height:25px'/><input type="hidden" name="bill_no[<?php echo $this->_tpl_vars['data']['tid']; ?>
]" value="<?php echo $this->_tpl_vars['data']['bill_no']; ?>
"/></td>
        <td><input type='text' value='<?php echo $this->_tpl_vars['data']['logistic_no']; ?>
' name='logistic_no[<?php echo $this->_tpl_vars['data']['tid']; ?>
]'></td>
...
"></td>
        ...
        <td><?php if ($this->_tpl_vars['data']['invoice_type'] > 0): ?>是<?php else: ?>否<?php endif; ?></td>
        <td><?php echo $this->_tpl_vars['data']['invoice_no']; ?>
</td>
