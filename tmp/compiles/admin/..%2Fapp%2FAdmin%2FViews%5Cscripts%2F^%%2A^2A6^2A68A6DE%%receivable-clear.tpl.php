<?php /* Smarty version 2.6.19, created on 2014-12-22 17:00:26
         compiled from finance/receivable-clear.tpl */ ?>
<?php if (! $this->_tpl_vars['param']['job']): ?>
    <div id="source_select" style="padding:10px">
    <form name="searchForm" id="searchForm" method="POST" action="<?php echo $this -> callViewHelper('url', array());?>" enctype="multipart/form-data" target="upload_file_frame">
    <input type="hidden" name="pay_type" value="<?php echo $this->_tpl_vars['param']['pay_type']; ?>
">
    支付类型：<b>
          <?php if ($this->_tpl_vars['param']['pay_type'] == 'alipay'): ?>支付宝
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'phonepay'): ?>手机支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'tenpay'): ?>财付通
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'bankcomm'): ?>交通银行
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'bank'): ?>银行打款
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'cash'): ?>现金支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'credit'): ?>赊销支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'distribution'): ?>直供支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'external'): ?>渠道支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'externalself'): ?>渠道代发货支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'sf'): ?>顺丰
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'ems'): ?>EMS
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'gift'): ?>礼品卡
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'point'): ?>积分
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'account'): ?>账户余额
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'exchange'): ?>换货支付
          <?php elseif ($this->_tpl_vars['param']['pay_type'] == 'easipay'): ?>东方支付
          <?php endif; ?></b><br>
...
<input type="button" name="settle" id="settle" value="结算" style="display:none" onclick="ajax_submit($('myForm1'),'<?php echo $this -> callViewHelper('url', array());?>')" />
</div>
